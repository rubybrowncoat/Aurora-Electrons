        ...
        <v-col cols="12">
          <v-data-table ref="tablle" :headers="headers" :items="tasks" item-key="ID" class="elevation-1" sort-by="RemainingDays" disable-pagination hide-default-footer multi-sort>
            ...
            <template v-slot:[`item.PopulationID`]="{ item }">
              ...
              <span v-if="populationProductionModifiers[item.PopulationID]">{{ systemBodyName(populationProductionModifiers[item.PopulationID]) }}</span>
            </template>
          </v-data-table>
        </v-col>
      </v-row>
