<template>
  <div class="fill-height">
    ...
    <v-container fluid>
      <v-row justify="start">
        <v-col cols="4">
          <v-autocomplete :background-color="$vuetify.theme.dark ? 'rgba(5, 5, 10, 0.75)' : 'rgba(250, 250, 255, 0.85)'"
            v-model="focusSystemId" :items="sortedNodes" item-text="name" item-value="id" label="Highlighted System"
            auto-select-first hide-selected dense clearable outlined></v-autocomplete>
        </v-col>
        <v-col cols="4">
        ...
        <v-col cols="4" class="d-flex justify-end">
          <v-checkbox v-model="useSavedPositions" label="Show saved positions" dense></v-checkbox>
        </v-col>
      </v-row>
    </v-container>
  </div>
...
const sectorColorChain = ["#e60049", "#0bb4ff", "#50e991", "#e6d800", "#9b19f5", "#ffa300", "#dc0ab4", "#b3d4ff", "#00bfa0", "#b30000", "#7c1158", "#4421af", "#1a53ff", "#0d88e6", "#00b7c7", "#5ad45a", "#8be04e", "#ebdc78", "#fd7f6f", "#7eb0d5", "#b2e061", "#bd7ebe", "#ffb55a", "#ffee65", "#beb9db", "#fdcce5", "#8bd3c7", "#ea5545", "#f46a9b", "#ef9b20", "#edbf33", "#ede15b", "#bdcf32", "#87bc45", "#27aeef", "#b33dc6"]
    ...
    initializeGraph() {
      ...
      this.graph
        ...
        .nodeCanvasObject((node, ctx, globalScale) => {
          ...
          if (node.sector) {
            ctx.fillStyle = sectorColorChain[node.sectorId % sectorColorChain.length]
            ctx.beginPath()
            ctx.arc(node.x, node.y, 5 * scaleFraction, 0, 2 * Math.PI)
            ctx.fill()
          }
