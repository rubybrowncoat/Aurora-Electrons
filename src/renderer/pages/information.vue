  ...
  <div>
    ...
    <div v-else>
      <v-container fluid>
        ...
        <v-row v-if="CivilianShippingLinesActive && activeInstallations.length" class="mt-8">
          <v-col cols="12">
            <div class="display-1">
              Civilian Network Work Orders
            </div>
          </v-col>
          <v-col v-for="installation of activeInstallations" :key="installation.PlanetaryInstallationID" cols="12">
            <v-card>
              <v-card-title>
                {{ installation.Name }}
              </v-card-title>
                  ...
                  <template #default>
                    ...
                    <tbody>
                      <tr v-for="demand of demandsAndDepartures[installation.PlanetaryInstallationID]" :key="`${demand.PopulationID}-${demand.PlanetaryInstallationID}`">
                        <td class="py-2">
                          <h3 v-html="populationName(demand)" />
                          <template v-if="demand.Departures.length">
                            <div v-for="cargo of demand.Departures" :key="`${cargo.PlanetaryInstallationID}-${cargo.OriginPopulationID}-${cargo.DestinationPopulationID}`">
                              <template v-if="cargo.DestinationPopulationID">{{ cargo.Amount }} units en route to <span v-html="populationName(cargo, 'Destination', true)" /></template>
                      ...
                      <tr v-if="installationBalance[installation.PlanetaryInstallationID]" class="caption">
                        <td>Unbalanced</td>
                        <td class="text-center font-weight-bold"><span v-if="installationBalance[installation.PlanetaryInstallationID] > 0" class="red--text">{{ installationBalance[installation.PlanetaryInstallationID] }}</span></td>
                        <td class="text-center font-weight-bold"><span v-if="installationBalance[installation.PlanetaryInstallationID] < 0" class="red--text">{{ Math.abs(installationBalance[installation.PlanetaryInstallationID]) }}</span></td>
                      </tr>
                    </tbody>
                  </template>
                </v-simple-table>
